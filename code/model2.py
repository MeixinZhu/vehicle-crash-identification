...
def predict_model2():
    print('Generate model2 predict')
    temp = np.load('../user_data/train_time_model2.npz')
    ...
    window = 10
    acc_list = []
    for vehno in tqdm(range(121, 261)):
        acc_flag = False
        acc_time = ''
        acc_speed = None
        acc_current = None
        ...
        # print('\r',[vehno, int(acc_flag), str(acc_time), max_prob], end = '')

    model_res = pd.DataFrame(acc_list, columns = ['vehno','Label','CollectTime', 'prob', 'acc_speed', 'acc_current'])
    out = model_res.rename(columns = {'vehno': '车号'})
    out = out[['车号', 'Label', 'CollectTime', 'prob']]
    out.to_csv('../user_data/model2.csv', index = False)
